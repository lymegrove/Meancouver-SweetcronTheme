    ...
    <ul id="activity_list">
        <?php if ($items): $i = 1; foreach ($items as $item): ?>
            ...
            <li class="item <?php echo $item->get_feed_class()?> <?php if ($i % 3 == 0): ?> last<?php endif; ?>">
            	<p class="site_info" style="background-size: 16px 16px;background:#000 url(<?php echo $item->get_feed_icon()?>) 3px center no-repeat">Posted at <a href="<?php echo $this->config->item('base_url')?>items/site/<?php echo $item->get_feed_domain()?>"><?php echo $item->get_feed_domain()?></a></p>
            	...
            	<?php elseif ($item->get_feed_domain() == 'careerjet.ca'): ?>
